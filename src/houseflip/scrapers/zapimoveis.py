"""ZapImóveis scraper — parses HTML listings (SSR, not __NEXT_DATA__)."""
...
_PROP_TYPE_SLUG = {
    ...
    PropertyType.COMMERCIAL: "comerciais",
}

_LISTING_TYPE_MAP = {
    "venda": ListingType.SALE,
    "aluguel": ListingType.RENT,
}
...
class ZapImoveisScraper(BaseScraper):
    ...
    def _parse_item(self, item: Selector) -> Listing | None:
        try:
            ...
            # Price — first <p> text, e.g. "R$ 1.190.000" or "A partir de R$ 1.190.000"
            ...
            area = Decimal(area_match.group().replace(",", ".")) if area_match else None
            ...
            # Location — "Vila Anastácio, São Paulo"
            ...
            logger.exception("ZapImóveis: erro ao parsear item")
            return None

    def _has_next_page(self, html: str, page: int) -> bool:
        sel = Selector(text=html)
        items = sel.css('li[data-cy="rp-property-cd"]')
        # ZapImóveis shows 30 items per page; fewer means last page
        return len(items) >= 30
